<script lang="ts">
	...
	page.subscribe((value) => {
		globalState.currentPage =
			value.data.pages.find((pageData: Page) => pageData.pathname === value.url.pathname) || null;
	});
	...
	const hidePageStates = ['mounting', 'navigating', 'navigatingWhileFlying', 'flying'];
	...
	const getPublicViewer = async () =>
		new Promise<PublicViewer>((resolve) => {
			...
			const unsubscribe = publicViewerStore.subscribe((value: null | PublicViewer) => {
				if (!value) {
					return;
				}
				unsubscribe();
...
<div class="relative flex h-dvh w-screen flex-col overflow-hidden">
	...
	{/if}
	<main class="text-foreground relative h-full w-full">
		<div id="container" class="relative h-full w-full">
			...
			></div>
			{#if showPage}
				<div class="relative top-0 h-full w-full" class:pointer-events-none={$page.data.isHome}>
					{@render children()}
				</div>
			{/if}
			{#if viewer}
