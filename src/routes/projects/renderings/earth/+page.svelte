...
<Project title={EARTH_LABEL} githubLink="https://github.com/PierreLeBlond/earth">
	{#snippet about()}
		<div class="flex w-full flex-col">
			{#if app.state !== 'idle'}
				<p in:fade={{ delay: 1 }}>
					Nothing appears yet, but you know some hidden works are happening. You are willing to
					wait...
				</p>
			{:else}
				<p in:fade={{ delay: 1 }}>
					It's a globe. It reflects the room it's in. There is some <b>three.js</b> magic happening for
					sure.
				</p>
				{#if country}
					<p in:fade={{ delay: 1 }}>
	...
	{#snippet excerpt()}
		<div>
			{#if country}
				<p in:fade={{ delay: 1 }} class="overflow-hidden text-nowrap text-ellipsis">
