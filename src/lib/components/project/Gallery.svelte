...
<div class="bg-background/20 relative flex max-w-96 justify-center overflow-x-hidden rounded-xl">
	<div
		class="horizontal:aspect-[1.5] flex aspect-[2/3] h-full max-w-full snap-x snap-mandatory overflow-x-auto scroll-smooth"
		bind:this={element}
	...
	>
		{#each screenshots as screenshot, index}
			...
			>
				<img src={screenshot} alt="teamup screenshot" class="m-0 aspect-[0.45] h-full shadow-lg" />
			</div>
		{/each}
	</div>
	<div class="pointer-events-none absolute flex aspect-[1.5] h-full max-w-full justify-between">
		<div class="top-0 left-0 flex h-full w-[20%] items-center justify-center">
			<button
		...
		<div class="top-0 right-0 flex h-full w-[20%] items-center justify-center">
			<button
				class="pointer-events-auto"
				onclick={() => scrollToScreenshot(1)}
